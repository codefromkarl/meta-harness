...
def _required_fields(expectations: dict[str, Any], extracted: Any) -> list[str]:
    configured = expectations.get("required_fields")
    if isinstance(configured, list):
        return [str(item) for item in configured if str(item)]
    if isinstance(extracted, dict):
        return sorted(_flatten_scalar_fields(extracted))
    return []


def _filled_required_field_rate(required_fields: list[str], extracted: Any) -> float:
    if not required_fields:
        return 0.0
    filled = 0
    for field in required_fields:
        value = _lookup_field(extracted, field)
        if value is None:
            continue
...
def _grounded_field_rate(required_fields: list[str], extracted: Any, page_text: str) -> float:
    if not page_text:
        return 0.0
    ...
    for value in present_values:
        ...
        if normalized_value and normalized_value in normalized_page:
            grounded += 1
...
def evaluate_web_scrape_run(run_dir: Path) -> dict[str, Any]:
    task_dirs = iter_task_dirs(run_dir)
    ...
    for item in relevant_tasks:
        ...
        extracted = read_json_if_exists(task_dir / "extracted.json")
        page_html = read_text_if_exists(task_dir / "page.html")
        schema_valid = isinstance(extracted, dict) and bool(extracted)
        schema_valid_rates.append(1.0 if schema_valid else 0.0)

        required_fields = _required_fields(expectations, extracted)
        completeness = (
            _filled_required_field_rate(required_fields, extracted) if schema_valid else 0.0
        )
        grounded = (
            _grounded_field_rate(required_fields, extracted, page_html) if schema_valid else 0.0
        )
        ...
        retry_value = _probe_value(probe_payload, "scrape.retry_count")
        navigation_value = _probe_value(probe_payload, "scrape.navigation_depth")
        if retry_value is not None:
            retry_values.append(retry_value)
        if navigation_value is not None:
            navigation_values.append(navigation_value)
        ...
        if transfer_task:
            ...
            binding_payload_present = 1.0 if isinstance(binding_payload, dict) and binding_payload else 0.0
    ...
    schema_valid_rate = average_numeric(schema_valid_rates)
    ...
    navigation_depth = average_numeric(navigation_values)
    ...
    if navigation_values:
        observed_probes["scrape.navigation_depth"] = navigation_depth
    if retry_values:
        observed_probes["scrape.retry_count"] = retry_count
    ...
    result = {
        ...
        "cost": {
            "web_scrape_latency_ms": latency_ms,
        },
        "architecture": {},
        "capability_scores": {
            "web_scrape": {
    ...
    if has_transfer_tasks:
        ...
        result["capability_scores"]["web_scrape"].update(
            {
        ...
        result["probes"].update(
            {
                "web_scrape.binding_payload_present_rate": binding_payload_rate,
                "web_scrape.assistant_reply_rate": assistant_reply_rate,
                "web_scrape.artifact_coverage_rate": artifact_coverage_rate,
                "web_scrape.binding_token_total": binding_token_total,
            }
        )
    return result
