...
- `next_round_context.json.artifacts` 必须是对象，用于承载 iteration 级 artifact link
...
- `artifacts.experience_summary_json` 必须指向当前 iteration 目录下的 `experience_summary.json`
- 若存在 `validation_summary.json`，则 `next_round_context.json.artifacts.validation_summary_json` 也必须指向它
```
