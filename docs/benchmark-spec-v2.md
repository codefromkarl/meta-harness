...
```json
{
  "experiment": "contextatlas_method_comparison",
...
- 让 ContextAtlas probe 脚本输出结构化方法信号

这一步主要改：

- `src/meta_harness/optimizer_workflow.py`
- `scripts/contextatlas_benchmark_probe.ts`
- `scripts/contextatlas_run_eval.py`
...
## 与当前 ContextAtlas 的对应关系

在 ContextAtlas 这条线上，V2 最值得先做的不是继续扫 `top_k`，而是把下面这些对象提升成方法级 variant：
