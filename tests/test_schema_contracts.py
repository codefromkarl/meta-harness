...
def test_loop_contract_validator_accepts_absolute_validation_summary_artifact_link(
    tmp_path: Path,
) -> None:
    loop_dir = loop_root_path(tmp_path / "reports", "loop-abs-validation-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-abs-validation-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-abs-validation-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-abs-validation-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-abs-validation-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-abs-validation-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-abs-validation-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
...
def test_loop_contract_validator_requires_benchmark_summary_artifact_link(
    tmp_path: Path,
) -> None:
    loop_dir = loop_root_path(tmp_path / "reports", "loop-missing-benchmark-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-missing-benchmark-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-missing-benchmark-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-missing-benchmark-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-missing-benchmark-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-missing-benchmark-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-missing-benchmark-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
    ...
    assert result["ok"] is False
    assert any(
        "next_round_context.json missing artifacts.benchmark_summary_json" in error
        for error in result["errors"]
    )
...
def test_loop_contract_validator_accepts_relative_benchmark_summary_artifact_link(
    ...
    loop_dir = loop_root_path(tmp_path / "reports", "loop-rel-benchmark-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-rel-benchmark-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-rel-benchmark-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-rel-benchmark-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-rel-benchmark-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-rel-benchmark-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-rel-benchmark-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
...
def test_loop_contract_validator_rejects_wrong_benchmark_summary_artifact_link(
    tmp_path: Path,
) -> None:
    loop_dir = loop_root_path(tmp_path / "reports", "loop-wrong-benchmark-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-wrong-benchmark-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-wrong-benchmark-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-wrong-benchmark-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-wrong-benchmark-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-wrong-benchmark-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-wrong-benchmark-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
    ...
    assert result["ok"] is False
    assert any(
        "next_round_context.json artifacts.benchmark_summary_json does not point to benchmark_summary.json"
        in error
        for error in result["errors"]
    )
...
def test_loop_contract_validator_rejects_wrong_experience_summary_artifact_link(
    tmp_path: Path,
) -> None:
    loop_dir = loop_root_path(tmp_path / "reports", "loop-wrong-experience-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-wrong-experience-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-wrong-experience-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-wrong-experience-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-wrong-experience-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-wrong-experience-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-wrong-experience-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
    ...
    assert result["ok"] is False
    assert any(
        "next_round_context.json artifacts.experience_summary_json does not point to experience_summary.json"
        in error
        for error in result["errors"]
    )


def test_loop_contract_validator_accepts_relative_experience_summary_artifact_link(
    tmp_path: Path,
) -> None:
    loop_dir = loop_root_path(tmp_path / "reports", "loop-rel-experience-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-rel-experience-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-rel-experience-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-rel-experience-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-rel-experience-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-rel-experience-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-rel-experience-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
...
def test_loop_contract_validator_accepts_absolute_experience_summary_artifact_link(
    tmp_path: Path,
) -> None:
    loop_dir = loop_root_path(tmp_path / "reports", "loop-abs-experience-artifact-link")
    iteration_dir = loop_dir / "iterations" / "loop-abs-experience-artifact-link-0001"
    iteration_dir.mkdir(parents=True, exist_ok=True)
    proposer_context_dir = iteration_dir / "proposer_context"
    proposer_context_dir.mkdir(parents=True, exist_ok=True)
    (proposer_context_dir / "manifest.json").write_text("{}", encoding="utf-8")
    ...
    (loop_dir / "loop.json").write_text(
        json.dumps(
            {
                "loop_id": "loop-abs-experience-artifact-link",
                "profile_name": "demo",
                "project_name": "demo",
                "request": {},
                "iteration_count": 1,
                "stop_reason": "done",
                "iterations": [{"iteration_id": "loop-abs-experience-artifact-link-0001"}],
            }
        ),
        encoding="utf-8",
    )
    (loop_dir / "iteration_history.jsonl").write_text(
        json.dumps({"iteration_id": "loop-abs-experience-artifact-link-0001"}) + "\n",
        encoding="utf-8",
    )
    for name, payload in {
        "iteration.json": {"iteration_id": "loop-abs-experience-artifact-link-0001"},
        "proposal_input.json": {"objective": {}, "experience": {}},
        "proposal_output.json": {"proposal": {}},
        "selected_candidate.json": {"candidate_id": "cand-1"},
        "benchmark_summary.json": {"evaluation": {"validation": {"status": "passed"}}},
        "validation_summary.json": {"status": "passed"},
        "experience_summary.json": {"iteration_id": "loop-abs-experience-artifact-link-0001"},
        "next_round_context.json": {
            "experience_summary_path": "experience_summary.json",
            "validation_summary_path": "validation_summary.json",
            "artifacts": {
